import { EmailValidator, EmailValidationResult, EmailValidationError } from './main';
...
describe('EmailValidator', () => {
  describe('validateEmail', () => {
    ...
    test('should reject email exceeding 320 characters', async () => {
      ...
      await expect(EmailValidator.validateEmail(email)).rejects.toThrow(EmailValidationError);
      await expect(EmailValidator.validateEmail(email)).rejects.toThrow('Email exceeds maximum length of 320 characters');
    });

    test('should throw error for non-string input', async () => {
      await expect(EmailValidator.validateEmail(123 as any)).rejects.toThrow(EmailValidationError);
      await expect(EmailValidator.validateEmail(123 as any)).rejects.toThrow('Email must be a string');
    });

    test('should throw error for null/undefined input', async () => {
      await expect(EmailValidator.validateEmail(null as any)).rejects.toThrow(EmailValidationError);
      await expect(EmailValidator.validateEmail(undefined as any)).rejects.toThrow(EmailValidationError);
    });
  ...
  describe('validateEmails', () => {
    ...
    test('should throw error for non-array input', async () => {
      await expect(EmailValidator.validateEmails('not an array' as any)).rejects.toThrow(EmailValidationError);
      await expect(EmailValidator.validateEmails('not an array' as any)).rejects.toThrow('Emails must be an array');
    });
  ...
  describe('Error Handling', () => {
    test('should create EmailValidationError with correct properties', () => {
      const error = new EmailValidationError('Test message', 'TestError', 'Test details');
